...
public class GroupMemberDAOImpl {
    ...
    public void save(GroupMember groupMember) {// 插入记录
        ...
        db.execSQL("insert into t_groupMember (userId,userName,userPortraitUri) " +
                "values(?,?,?)",
                new Object[] { groupMember.getUserId(), groupMember.getUserName(), groupMember.getUserPortraitUrl() });
        db.close();  //关闭数据库操作
    }
    ...
    public GroupMember find(String id) {// 根据ID查找纪录
        ...
        if (cursor.moveToFirst()) {// 依次取出数据
            ...
            groupMember.setUserPortraitUrl(cursor.getString(cursor.getColumnIndex("userPortraitUri")));
        }
        db.close();
        return groupMember;
    }
