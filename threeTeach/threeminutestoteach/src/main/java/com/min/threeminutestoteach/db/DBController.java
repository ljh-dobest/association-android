package com.min.threeminutestoteach.db;
...
import com.min.threeminutestoteach.domain.MyDownloadInfLocal;
import com.min.threeminutestoteach.domain.MyDownloadThreadInfoLocal;
...
public class DBController implements DownloadDBController {
  ...
  @Override
  public List<DownloadInfo> findAllDownload() {
    try {
      List<MyBusinessInfLocal> myBusinessInfLocals = myBusinessInfoLocalsDao.queryForAll();
      DataUtils.DOWNLOAD_LIST.clear();
      DataUtils.DOWNLOAD_LIST.addAll(myBusinessInfLocals);
      return convertDownloadInfos1(myBusinessInfLocals);
    } catch (SQLException e) {
      e.printStackTrace();
    }
    return new ArrayList<>();
  ...
  private List<DownloadInfo> convertDownloadInfos1(List<MyBusinessInfLocal> infos) {
    List<DownloadInfo> downloadInfos = new ArrayList<>();
    for (MyBusinessInfLocal downloadInfLocal : infos) {
      downloadInfos.add(convertDownloadInfo1(downloadInfLocal));
    }
    return downloadInfos;
  }
  private DownloadInfo convertDownloadInfo1(MyBusinessInfLocal downloadInfLocal) {
    if (downloadInfLocal == null) {
      return null;
    }
    DownloadInfo downloadInfo = new DownloadInfo();
    downloadInfo.setUri(downloadInfLocal.getUrl());
    return downloadInfo;
  }
